(ns mongodb-async.aggregation
  ...
  (:import [com.mongodb Block]
           ...
           [com.mongodb.async.client
            ...
            FindIterable
            MongoClientSettings]
           [com.mongodb.client.model Aggregates Accumulators Projections BsonField]
...
(defn aggregate [^MongoCollection coll]
  (.forEach (.aggregate coll [(Aggregates/match (eq "categories" "Pastries"))
                              (Aggregates/group "$stars" (into-array BsonField [(Accumulators/sum "count" 1)]))])
            (proxy [Block] []
              (apply [^Document doc]
                (println (.toJson doc))))
                               ...
                               (Projections/fields
                                (into-array Bson
                                            [(Projections/excludeId)
                                             (Projections/include (into-array String ["name"]))
